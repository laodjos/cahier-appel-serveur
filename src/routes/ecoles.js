...
// PATCH /api/ecoles/:id/suspension  { suspendue }
// Bascule manuellement l'accès d'un établissement, indépendamment de sa date de
// fin d'utilisation — pour bloquer immédiatement en attente de règlement.
router.patch("/:id/suspension", requireRole("super_admin"), async (req, res) => {
  const { suspendue } = req.body;
  const { rows } = await pool.query(
    "UPDATE ecoles SET suspendue = $1 WHERE id = $2 RETURNING *",
    [!!suspendue, req.params.id]
  );
  if (!rows[0]) return res.status(404).json({ error: "École introuvable." });
  res.json(rows[0]);
});

// PATCH /api/ecoles/:id/erp  { erp_actif }
// Active/désactive le module ERP (notes/bulletins + frais de scolarité) pour un
// établissement — option payante séparée de l'abonnement de base.
router.patch("/:id/erp", requireRole("super_admin"), async (req, res) => {
  const { erp_actif } = req.body;
  const { rows } = await pool.query(
    "UPDATE ecoles SET erp_actif = $1 WHERE id = $2 RETURNING *",
    [!!erp_actif, req.params.id]
  );
  if (!rows[0]) return res.status(404).json({ error: "École introuvable." });
  res.json(rows[0]);
});
